...
namespace MetaPassRenderingV2
{
    public class MetaPassRenderingV2
    {
        ...
        private static string localAssetFolder = "Assets/MetaPassRenderingV2";
        private static string localAssetShadersFolder = "Assets/MetaPassRenderingV2/Shaders";
        private static string dilateAssetPath = "Assets/MetaPassRenderingV2/Shaders/Dilation.compute";
        private static string dataPackingAssetPath = "Assets/MetaPassRenderingV2/Shaders/DataPacking.compute";

        private ComputeShader dilate;
        private ComputeShader dataPacking;
        private Shader sceneObjectShader;
        private Material sceneObjectMaterial;
        private MaterialPropertyBlock sceneObjectMaterialPropertyBlock;
        private RenderTextureConverterV2 renderTextureConverter;

        //|||||||||||||||||||||||||||||||||||||| SETUP ||||||||||||||||||||||||||||||||||||||
        //|||||||||||||||||||||||||||||||||||||| SETUP ||||||||||||||||||||||||||||||||||||||
        //|||||||||||||||||||||||||||||||||||||| SETUP ||||||||||||||||||||||||||||||||||||||

        public MetaPassRenderingV2()
        {
            ...
            sceneObjectShader = Shader.Find("MetaPassRenderingV2/SceneObjectShader");
        ...
        public List<ObjectMetaData> ExtractSceneObjectMetaBuffers()
        {
            ...
            dilate.SetInt(ShaderIDs.KernelSize, dilationPixelSize);
            ...
            for (int i = 0; i < meshRenderers.Length; i++)
            {
                ...
                if (onlyUseMeshesWithinBounds)
                    includeMesh = includeMesh && ContainBounds(sceneObjectsBounds, meshRenderer.bounds);
                        ...
                        for (int j = 0; j < materials.Length; j++)
                        {
                            ...
                            if (metaPassIndex != -1)
                            {
                                ...
                                if(performDilation)
                                {
                                    ...
                                    dilate.SetTexture(ComputeShader_Dilation, ShaderIDs.Write, albedoBuffer);
                                ...
                                {
                                    //reuse the same buffer, the compute shader will modify the values of this render target.
                                    dilate.SetTexture(ComputeShader_Dilation, ShaderIDs.Write, emissiveBuffer);
                                ...
                                dataPacking.SetTexture(ComputeShader_DataPacking64, ShaderIDs.AlbedoBuffer, albedoBuffer);
                                dataPacking.SetTexture(ComputeShader_DataPacking64, ShaderIDs.EmissiveBuffer, emissiveBuffer);
        ...
        public void CleanUpSceneObjectMetaBuffers(List<ObjectMetaData> objectsMetaData)
        {
            for (int i = 0; i < objectsMetaData.Count; i++)
        ...
        public void RenderScene(List<ObjectMetaData> sceneObjectMetaData, Camera camera, RenderTexture renderTexture)
        {
            ...
            using (CommandBuffer sceneCommandBuffer = new CommandBuffer())
            {
                ...
                {
                    ObjectMetaData objectMetaData = sceneObjectMetaData[i];
        ...
        public void UnpackSceneRender(RenderTexture packedRenderTexture, RenderTexture albedoRenderTexture, RenderTexture emissiveRenderTexture, RenderTexture normalRenderTexture)
        {
            ...
            dataPacking.SetTexture(ComputeShader_DataUnpacking64, ShaderIDs.PackedBuffer, packedRenderTexture);
            dataPacking.SetTexture(ComputeShader_DataUnpacking64, ShaderIDs.AlbedoBuffer, albedoRenderTexture);
            dataPacking.SetTexture(ComputeShader_DataUnpacking64, ShaderIDs.EmissiveBuffer, emissiveRenderTexture);
            dataPacking.SetTexture(ComputeShader_DataUnpacking64, ShaderIDs.NormalBuffer, normalRenderTexture);

            dataPacking.Dispatch(ComputeShader_DataUnpacking64, Mathf.CeilToInt(packedRenderTexture.width / THREAD_GROUP_SIZE_X), Mathf.CeilToInt(packedRenderTexture.height / THREAD_GROUP_SIZE_Y), 1);
